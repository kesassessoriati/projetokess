...
const buildOpportunityScope = ({
  ...
  Request,
  "pipelineId" | "companyId" | "profile" | "userId" | "ownerUserId" | "viewMode"
>) => {
  const where: any = {
    pipelineId,
    companyId,
...
const buildKeywordScope = async (companyId: number, searchKeyword?: string) => {
  ...
  const opportunityIds = eventRows
  ...
  if (contactIds.length > 0)
    conditions.push({ contactId: { [Op.in]: contactIds } });
  if (opportunityIds.length > 0)
    conditions.push({ id: { [Op.in]: opportunityIds } });

  return { [Op.or]: conditions };
};

const buildPredictionInclude = (
  ...
  if (filter?.riskLevel || filter?.minProbability) {
    ...
    if (filter.minProbability) {
      predictionInclude.where.predictedCloseProbability = {
        [Op.gte]: filter.minProbability
      };
    }
  }
...
const hasInvalidOpportunityIdentity = (op: Opportunity): boolean =>
  !op.contact?.id && !op.lead?.contactId && !op.lead?.phone;
...
const ListPipelineBoardService = async ({
  ...
  if (keywordScope) {
    scopedOpportunityWhere[Op.and] = [
      ...(Array.isArray(scopedOpportunityWhere[Op.and])
        ? scopedOpportunityWhere[Op.and]
        : []),
      keywordScope
    ];
  ...
  const scopedStatsWhere = applyBoardFilters(
    ...
    include: [
      {
        model: CrmLead,
        as: "lead",
        attributes: ["id", "contactId", "phone", "purchaseValue"],
        where: { companyId },
        required: false
      },
      {
  ...
  ).filter(op => !hasInvalidOpportunityIdentity(op));

  const statsMap = uniqueStatsRows.reduce((acc, op) => {
    const stageKey = op.stageId;
    const fallbackLeadValue =
      op.lead?.purchaseValue != null ? Number(op.lead.purchaseValue) : 0;
  ...
  const effectiveLimit = normalizeKeyword(searchKeyword)
    ? Math.max(limit, 1000)
    : limit;
  ...
  const getOpportunitiesForStage = async (sId: number, sCursor?: string) => {
    ...
    if (sCursor) {
      const [createdAt, id] = Buffer.from(sCursor, "base64")
        .toString("ascii")
        .split("_");
      where[Op.or] = [
    ...
    const results = hasMore
      ? opportunities.slice(0, effectiveLimit)
      : opportunities;
  ...
  const boardStages: BoardStage[] = await Promise.all(
    pipeline.stages.map(async stage => {
      ...
      if (shouldLoadOps) {
        opsData = await getOpportunitiesForStage(
          stage.id,
          stageId === stage.id ? cursor : undefined
        );
      }

      const stageStats = statsMap[stage.id] || {
        count: 0,
        totalValue: 0,
        forecastValue: 0,
        highRiskCount: 0
      ...
      return {
        ...
        opportunities: opsData.results.flatMap(op => {
          const now = new Date();
          let slaStatus: "NORMAL" | "EXPIRED" | "CRITICAL" = "NORMAL";
          const fallbackLeadValue =
            op.lead?.purchaseValue != null ? Number(op.lead.purchaseValue) : 0;
          const opportunityValue = Number(op.value || 0);
          const invalidIdentity = hasInvalidOpportunityIdentity(op);
          const effectiveValue =
            opportunityValue === 0 && fallbackLeadValue > 0
              ? fallbackLeadValue
              : opportunityValue;
          ...
          if (invalidIdentity) {
            logger.warn(
              "[KANBAN_DEDUPE] invalid opportunity identity on board",
              {
                companyId,
  ...
  const scheduledMeetingsCount = await CrmLead.count({
    where: {
      ...
      [Op.or]: [
        ...
        ...(scheduledMeetingStageIds.length > 0
          ? [{ stageId: { [Op.in]: scheduledMeetingStageIds } }]
          : [])
      ]
    }
  });
