  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      const fetchContacts = async () => {
        try {
          const { data } = await api.get(`/kanban-automations/${id}`);

          if (data.estrutura_fluxo !== null) {
            const flowNodes = data.estrutura_fluxo.nodes || [];
            if (flowNodes.length > 0) {
              setNodes(applyTitlesToNodes(flowNodes));
            }
            if (data.estrutura_fluxo.connections) {
              setEdges(data.estrutura_fluxo.connections);
            }
