...
const useStyles = makeStyles((theme) => ({
  root: { height: "100%", minHeight: "calc(100vh - 112px)", background: "radial-gradient(circle at top left, #effaf2 0%, #e5f2e9 42%, #ddebe3 100%)", padding: 14, overflowY: "auto", overflowX: "hidden", [theme.breakpoints.down("sm")]: { minHeight: "calc(100vh - 64px - 70px)", padding: 8 } },
  shell: { height: "100%", display: "flex", flexDirection: "column", gap: 10 },
  header: { borderRadius: 16, border: "1px solid #cfe2d5", background: "#ffffffeb", boxShadow: "0 10px 24px rgba(16,24,40,0.07)", padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", [theme.breakpoints.down("sm")]: { padding: "10px 12px", flexDirection: "column", alignItems: "stretch" } },
  ...
  summary: { display: "grid", gridTemplateColumns: "repeat(6,minmax(0,1fr))", gap: 10, [theme.breakpoints.down("sm")]: { gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 6 } },
  ...
  actionsBar: { display: "flex", gap: 8, flexWrap: "wrap", padding: 10, border: "1px solid #cedfd3", borderRadius: 14, background: "#ffffffdc", [theme.breakpoints.down("sm")]: { "& > *": { flex: "1 1 calc(50% - 4px)" } } },
  ...
  panelHead: { padding: "10px 12px", borderBottom: "1px solid #edf3ef", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" },
  panelTitle: { fontSize: ".84rem", fontWeight: 800, color: "#173624" },
  toolbar: { display: "grid", gridTemplateColumns: "2fr 1.5fr .7fr .7fr auto auto", gap: 8, padding: 12, borderBottom: "1px solid #edf3ef", [theme.breakpoints.down("sm")]: { gridTemplateColumns: "1fr 1fr", "& > *:nth-child(n+5)": { gridColumn: "span 1" } } },
  ...
  list: { maxHeight: "60vh", overflowY: "auto", padding: 8, [theme.breakpoints.down("sm")]: { maxHeight: "none" } },
    ...
    <Box className={classes.root}>
      <Box className={classes.shell}>
        ...
        <Box className={classes.content}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={8}>
                <Paper className={classes.panel}><Box className={classes.panelHead}><Typography className={classes.panelTitle}>Campanhas de grupos</Typography></Box><Box className={classes.form}>
                  <TextField variant="outlined" size="small" label="Nome" value={campaignForm.name} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, name: v })); }} />
                  <FormControl variant="outlined" size="small"><InputLabel>Conexao</InputLabel><Select value={campaignForm.whatsappId} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, whatsappId: v, groupIds: [] })); }} label="Conexao">{connections.map((c) => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}</Select></FormControl>
                  <FormControl variant="outlined" size="small"><InputLabel>Template</InputLabel><Select value={campaignForm.templateId} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, templateId: v })); }} label="Template"><MenuItem value="">Nenhum</MenuItem>{templates.map((t) => <MenuItem key={t.id} value={t.id}>{t.name}</MenuItem>)}</Select></FormControl>
                  <FormControl variant="outlined" size="small"><InputLabel>Tipo de conteudo</InputLabel><Select value={campaignForm.messageType} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, messageType: v, mediaContent: null })); }} label="Tipo de conteudo"><MenuItem value="text">Texto</MenuItem><MenuItem value="imagem">Imagem</MenuItem><MenuItem value="video">Video</MenuItem><MenuItem value="audio">Audio</MenuItem><MenuItem value="documento">Documento</MenuItem></Select></FormControl>
                  {["imagem", "video", "audio", "documento"].includes(campaignForm.messageType) ? <input key={campaignFileKey} type="file" onChange={(e) => { const f = readFile(e); setCampaignForm((p) => ({ ...p, mediaContent: f })); }} /> : null}
                  <TextField variant="outlined" size="small" label="Mensagem" multiline rows={4} value={campaignForm.message} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, message: v })); }} />
                  <FormControl variant="outlined" size="small"><InputLabel>Mencionar membros</InputLabel><Select value={campaignForm.mentionsMode} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, mentionsMode: v })); }} label="Mencionar membros"><MenuItem value="none">Nao mencionar</MenuItem><MenuItem value="all">Mencionar todos</MenuItem></Select></FormControl>
                  <FormControl variant="outlined" size="small"><InputLabel>Grupos alvo</InputLabel><Select multiple value={campaignForm.groupIds} onChange={(e) => { const v = readValue(e, []) || []; setCampaignForm((p) => ({ ...p, groupIds: v })); }} label="Grupos alvo" renderValue={(selected) => <Box style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{(selected || []).map((value) => <Chip key={value} size="small" label={campaignGroups.find((g) => Number(g.groupId) === Number(value))?.subject || value} />)}</Box>}>{campaignGroups.map((g) => <MenuItem key={g.groupId} value={g.groupId}>{g.subject}</MenuItem>)}</Select></FormControl>
                  <Box style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><TextField type="number" variant="outlined" size="small" label="Intervalo entre grupos (segundos)" value={campaignForm.intervalSeconds} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, intervalSeconds: v })); }} /><FormControl variant="outlined" size="small"><InputLabel>Tipo de envio</InputLabel><Select value={campaignForm.scheduleMode} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, scheduleMode: v })); }} label="Tipo de envio"><MenuItem value="now">Enviar agora</MenuItem><MenuItem value="scheduled">Agendar envio</MenuItem></Select></FormControl></Box>
                  {campaignForm.scheduleMode === "scheduled" ? <Box style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><TextField type="datetime-local" variant="outlined" size="small" label="Data e hora" InputLabelProps={{ shrink: true }} value={campaignForm.scheduledAt} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, scheduledAt: v })); }} /><FormControl variant="outlined" size="small"><InputLabel>Recorrencia</InputLabel><Select value={campaignForm.recurrenceRule} onChange={(e) => { const v = readValue(e); setCampaignForm((p) => ({ ...p, recurrenceRule: v })); }} label="Recorrencia"><MenuItem value="none">Unico</MenuItem><MenuItem value="daily">Diario</MenuItem><MenuItem value="weekly">Semanal</MenuItem></Select></FormControl></Box> : null}
          ...
          {!loading && tab === 4 ? <Paper className={classes.panel}><Box className={classes.panelHead}><Typography className={classes.panelTitle}>Templates</Typography></Box><Box className={classes.form}><TextField variant="outlined" size="small" label="Nome do template" value={templateForm.name} onChange={(e) => { const v = readValue(e); setTemplateForm((p) => ({ ...p, name: v })); }} /><FormControl variant="outlined" size="small"><InputLabel>Tipo</InputLabel><Select value={templateForm.messageType} onChange={(e) => { const v = readValue(e); setTemplateForm((p) => ({ ...p, messageType: v, mediaContent: null })); }} label="Tipo"><MenuItem value="text">Texto</MenuItem><MenuItem value="imagem">Imagem</MenuItem><MenuItem value="video">Video</MenuItem><MenuItem value="audio">Audio</MenuItem><MenuItem value="documento">Documento</MenuItem></Select></FormControl>{["imagem", "video", "audio", "documento"].includes(templateForm.messageType) ? <input key={templateFileKey} type="file" onChange={(e) => { const f = readFile(e); setTemplateForm((p) => ({ ...p, mediaContent: f })); }} /> : null}<TextField variant="outlined" size="small" multiline rows={3} label="Mensagem" value={templateForm.message} onChange={(e) => { const v = readValue(e); setTemplateForm((p) => ({ ...p, message: v })); }} /><Button className={classes.primaryBtn} startIcon={<AddIcon />} onClick={saveTemplate}>Salvar template</Button></Box><Box className={classes.list}>{templates.map((t) => <Box key={t.id} className={classes.row} style={{ cursor: "default" }}><Typography>{t.name || "Sem nome"}</Typography><Button size="small" className={classes.secondaryBtn} onClick={() => api.delete(`/group-management/templates/${t.id}`).then(refreshAll)}>Excluir</Button></Box>)}</Box></Paper> : null}
          {!loading && tab === 5 ? simpleList("Historico", historyLogs, "message") : null}
          {!loading && tab === 6 ? simpleList("Relatorios", reports?.campaigns || []) : null}
        </Box>
      </Box>

      <Dialog open={dialogs.batch} onClose={() => setDialogs((d) => ({ ...d, batch: false }))} fullWidth maxWidth="sm"><DialogTitle>Criar grupos em massa</DialogTitle><DialogContent style={{ display: "grid", gap: 12, paddingTop: 8 }}><FormControl variant="outlined" size="small" fullWidth><InputLabel>Conexao</InputLabel><Select value={batchForm.whatsappId} onChange={(e) => { const v = readValue(e); setBatchForm((p) => ({ ...p, whatsappId: v })); }} label="Conexao">{connections.map((c) => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}</Select></FormControl><TextField variant="outlined" size="small" label="Nome base" value={batchForm.baseName} onChange={(e) => { const v = readValue(e); setBatchForm((p) => ({ ...p, baseName: v })); }} /><TextField variant="outlined" size="small" type="number" label="Quantidade" value={batchForm.quantity} onChange={(e) => { const v = readValue(e); setBatchForm((p) => ({ ...p, quantity: v })); }} /><TextField variant="outlined" size="small" label="Participantes iniciais" multiline rows={4} value={batchForm.participants} onChange={(e) => { const v = readValue(e); setBatchForm((p) => ({ ...p, participants: v })); }} placeholder="Separe por virgula ou quebra de linha" /></DialogContent><DialogActions><Button onClick={() => setDialogs((d) => ({ ...d, batch: false }))}>Cancelar</Button><Button color="primary" variant="contained" onClick={handleBatchCreate}>Criar</Button></DialogActions></Dialog>
      <Dialog open={dialogs.bulkMembers} onClose={() => setDialogs((d) => ({ ...d, bulkMembers: false }))} fullWidth maxWidth="sm"><DialogTitle>Adicionar membros em massa</DialogTitle><DialogContent style={{ display: "grid", gap: 12, paddingTop: 8 }}><FormControl variant="outlined" size="small" fullWidth><InputLabel>Conexao</InputLabel><Select value={bulkMembersForm.whatsappId || selectedGroup?.whatsappId || selectedConnection} onChange={(e) => { const v = readValue(e); setBulkMembersForm((p) => ({ ...p, whatsappId: v, groupId: "" })); }} label="Conexao">{connections.map((c) => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}</Select></FormControl><FormControl variant="outlined" size="small" fullWidth><InputLabel>Selecionar grupo</InputLabel><Select value={bulkMembersForm.groupId || selectedGroup?.groupId || ""} onChange={(e) => { const v = readValue(e); setBulkMembersForm((p) => ({ ...p, groupId: v })); }} label="Selecionar grupo">{groups.filter((g) => g.whatsappId === Number(bulkMembersForm.whatsappId || selectedGroup?.whatsappId || selectedConnection)).map((g) => <MenuItem key={g.groupId} value={g.groupId}>{g.subject}</MenuItem>)}</Select></FormControl><FormControl variant="outlined" size="small" fullWidth><InputLabel>Estrategia</InputLabel><Select value={bulkMembersForm.strategy} onChange={(e) => { const v = readValue(e); setBulkMembersForm((p) => ({ ...p, strategy: v })); }} label="Estrategia"><MenuItem value="round_robin">Distribuir entre grupos</MenuItem><MenuItem value="all">Adicionar em todos os grupos</MenuItem></Select></FormControl><TextField variant="outlined" size="small" label="Contatos" multiline rows={6} value={bulkMembersForm.members} onChange={(e) => { const v = readValue(e); setBulkMembersForm((p) => ({ ...p, members: v })); }} placeholder="5511999999999&#10;5511888888888" /></DialogContent><DialogActions><Button onClick={() => setDialogs((d) => ({ ...d, bulkMembers: false }))}>Cancelar</Button><Button color="primary" variant="contained" onClick={handleBulkMembers}>Adicionar</Button></DialogActions></Dialog>
