...
const Dashboard = () => {
  ...
  const {
    ...
    setData: setDashboardMetrics,
    request: fetchDashboard
  } = useSafeApi("/dashboard", { manual: true });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Carregar métricas do dashboard via useSafeApi - Mês atual
        ...
        // Carregar dados dos últimos 30 dias - por semana
        const dayPromises = [];
        const labelsArr = [];
        ...
        for (let i = 5; i >= 0; i--) {
          ...
          labelsArr.push(moment().subtract(i * 5, 'days').format('DD/MM'));
          dayPromises.push(find({ date_from: periodStart, date_to: periodEnd }).catch(() => null));
        }
  ...
  return (
    <div className={classes.container}>
      <SafeComponent
        loading={loadingDashboard && isEmpty(counters)}
        error={errorDashboard}
        data={dashboardMetrics}
        renderData={() => (
          <>
      ...
      {/* Seção de Tutoriais (Fica fora do SafeComponent se quiser que apareça mesmo sem métricas, ou dentro se for dependente) */}
