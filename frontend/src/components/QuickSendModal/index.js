...
export default function QuickSendModal({
  ...
  const appendToken = (currentValue, setValue, token) => {
    const safeCurrent = String(currentValue || "");
    const spacer =
    ...
    <>
      ...
      <Box className={classes.infoBox} style={{ marginTop: 8 }}>
        <Typography style={{ fontSize: 11, color: "#15803d" }}>
          O disparo rapido agora entende variaveis como{" "}
          <strong>{"{{firstName}}"}</strong> e <strong>{"{{ms}}"}</strong> para
          personalizar cada mensagem automaticamente.
        </Typography>
      </Box>
    </>
  );
    ...
    const load = async () => {
      try {
        ...
        const availableContactLists =
          contactListRes.status === "fulfilled" &&
  ...
  // ── Helpers de botões ──────────────────────────────────────────────────────
  const addButton = () => {
    if (buttons.length < 4) setButtons([...buttons, { ...DEFAULT_BUTTON }]);
  };
  const removeButton = (i) =>
    setButtons(
      buttons.filter((_, idx) => idx !== i).length
        ? buttons.filter((_, idx) => idx !== i)
        : [{ ...DEFAULT_BUTTON }],
    );
  ...
  const addCard = () => {
    ...
    setCarouselCards([
      ...carouselCards,
      {
        ...
        footer: "",
        buttons: [
          { displayText: "Saiba mais", type: "reply", value: "btn_info" },
        ],
      },
    ]);
  };
  ...
  const removeCardButton = (ci, bi) => {
    ...
    next[ci] = {
      ...next[ci],
      buttons: (next[ci].buttons || []).filter((_, i) => i !== bi),
    };
    setCarouselCards(next);
  };
  ...
  // ── Helpers de enquete ─────────────────────────────────────────────────────
  const addPollOption = () => {
    if (pollOptions.length < 12) setPollOptions([...pollOptions, ""]);
  };
  ...
  const addListSection = () => {
    const currentRows = listSections.reduce(
    ...
    setListSections([
      ...
      {
        title: "Nova seção",
        rows: [{ title: "", rowId: `row_${Date.now()}`, description: "" }],
      },
    ]);
  };
  ...
  const addListRow = (si) => {
    ...
    next[si] = {
      ...
      rows: [
        ...(next[si].rows || []),
        { title: "", rowId: `row_${Date.now()}`, description: "" },
      ],
    };
    setListSections(next);
  };
  ...
  const totalListRows = listSections.reduce(
    (acc, section) => acc + (section.rows || []).length,
    0,
  );
  const shouldUseCampaignFlow =
    recipientMode !== "single" || deliveryMode === "scheduled";
  ...
  const carouselValid =
    carouselCards.length >= 1 && carouselCards.every((c) => c.body.trim());

  const pollValid =
    pollName.trim().length > 0 &&
  ...
  const listValid =
    ...
    listSections.every((section) =>
      (section.rows || []).every(
        (row) => row.title.trim().length > 0 && row.rowId.trim().length > 0,
      ),
    );

  const messagePayloadValid = (() => {
    if (messageType === "text")
      return message.trim().length > 0 || medias.length > 0;
    if (messageType === "buttons")
      return message.trim().length > 0 && buttonsValid;
  ...
  const handleSend = async () => {
    ...
    try {
      const numberToSend =
        numberValidation.normalizedNumber || normalizedNumber;
      ...
      if (messageType === "text") {
        ...
        formData.append(
          "pollOptions",
          JSON.stringify(pollOptions.map((o) => o.trim()).filter(Boolean)),
        );
    ...
    try {
      ...
      if (messageType === "text") {
        ...
        if (medias[0]) {
          ...
          if (medias.length > 1) {
            toast.info(
              "No disparo em massa, somente o primeiro anexo sera usado.",
            );
          }
        }
      } else if (messageType === "buttons") {
